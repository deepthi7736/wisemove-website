...
(function(){
  ...
  function initHeroAnimation(){
    ...
    heroLines.forEach((line, i) => {
      setTimeout(() => line.classList.add('in'), 260 + i * 140);
    });
    setTimeout(() => { if(visual) visual.classList.add('in'); }, 620);
  }

  /* ---------------------------------------------------------------------
     HOW WE MOVE (v2) — timeline fill plays once when it enters view.
     --------------------------------------------------------------------- */
  function initHowWeMove(){
    const timeline = document.getElementById('hwm2Timeline');
    if(!timeline) return;
  ...
  /* ---------------------------------------------------------------------
     HERO PARALLAX — subtle, multi-layer pointer response. Desktop only,
     disabled for touch and reduced-motion. Movement is capped small.
     --------------------------------------------------------------------- */
  function initHeroParallax(){
    ...
    if(!visual) return;
    const layers = [
      { el: visual.querySelector('.hv-idea'), strength: 10 },
      { el: visual.querySelector('.hv-getvia'), strength: 14 },
      { el: visual.querySelector('.hv-vashq'), strength: 14 },
      { el: visual.querySelector('.hv-engine'), strength: 16 },
    ].filter(l => l.el);
    ...
    visual.addEventListener('mousemove', (e) => {
      ...
      requestAnimationFrame(() => {
        ...
        layers.forEach(({ el, strength }) => {
          el.style.setProperty('--px', (relX * strength).toFixed(1) + 'px');
          el.style.setProperty('--py', (relY * strength).toFixed(1) + 'px');
